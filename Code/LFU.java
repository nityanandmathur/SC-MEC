...
class Image
{
    ...
    int frequency; //Stores the timestamp of the service
    ...
    public Image(String job_id, int frequency, long task_index, double cpu, double ram, double runtime_disk, double storage)
...
class Cache
{
    ...
    String min_frequency(HashMap<String,Image> hm)
    { // Function to find service id that has minimum timestamp from the set of images
        ...
        {
            if(entry.getValue().frequency < freq)
            {
                freq = entry.getValue().frequency;
                min_freq = entry.getKey();
            }
        }
        return min_freq;
    }
...
class LFU_cloud
{
    void LFU_cache(Cache cache, String drivers, String requests) throws IOException
    {
        ...
        while(sc.hasNextLine() && (count < cache.no_services_max))
        {
            ...
            else
            {
                ...
                while(!((CPU_req + CPU_copy <= cache.CPU_max) && (RAM_req + RAM_copy <= cache.RAM_max) && (Disk_storage_req + Disk_storage_curr <= cache.Disk_storage_max)))
                {
                    ...
                    {
                        String to_remove = cache.min_frequency(Running_clone); //finding service id with least time stamp
                    ...
                    if(Disk_storage_req + Disk_storage_curr > cache.Disk_storage_max)
                    {
                        ...
                        {
                            String to_remove = cache.min_frequency(Running_clone); //finding service id with minimum timestamp
                            Image remove = Running_clone.get(to_remove);
                        ...
                        {
                            String to_remove = cache.min_frequency(Free_clone); //finding service with minimum timestamp
                            Image remove = Running_clone.get(to_remove);
                            Runtime_copy -= remove.Runtime_disk;
                            Storage_copy -= remove.Storage;
                ...
                cache.Running.put(service_id, new Image(tokens[0], 1, Long.parseLong(tokens[2]), Double.parseDouble(tokens[3]), Double.parseDouble(tokens[4]), Double.parseDouble(tokens[5]), Storage_req));
        ...
        FileWriter writer = new FileWriter("F:/Service Cache Implementation/Experiments/LFU/1.1/Results.csv",true);
...
public class LFU
{
    public static void main(String[] args) throws IOException 
    {
        ...
        LFU_cloud e = new LFU_cloud();
        e.LFU_cache(c, drivers, requests);
        //c.print_cache(c);
        in.close();
    }
}