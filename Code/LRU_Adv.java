...
class Image {
    ...
    public Image(String job_id, long timestamp, long task_index, double cpu, double ram, double runtime_disk,
            double storage) { // Constructor to initialize instance of Image class
        this.job_id = job_id;
...
class Cache {
    ...
    String min_timestamp(HashMap<String, Image> hm) { // Function to find service id that has minimum timestamp from the
                                                      // set of images
        String min_ts = "";
...
class LRU {
    Cache replacement(Cache cache, Image new_service) {
        ...
        while (!((new_service.CPU + CPU_copy <= cache.CPU_max) && (new_service.RAM + RAM_copy <= cache.RAM_max)
            ...
            if ((new_service.CPU + cache.CPU_curr > cache.CPU_max) || (new_service.RAM + cache.RAM_curr > cache.RAM_max)) {
    ...
    void LRU_cache(Cache cache, String drivers, String requests) throws IOException {
        ...
        while (sc.hasNextLine() && (count < cache.no_services_max)) {
            ...
            Image new_service = new Image(tokens[0], Long.parseLong(tokens[1]), ts.getTime(), CPU_req, RAM_req,
                    Runtime_disk_req, Storage_req);
            if (cache.Running.containsKey(service_id)) // if service is cached
            {
                cache_hit++;
                Image to_update = cache.Running.get(service_id);
                to_update.timestamp = ts.getTime(); // update its timestamp
                ...
                {
                    cache = this.replacement(cache, new_service);
                }
            } 
            else 
            {
