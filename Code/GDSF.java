...
class Image
{
    ...
    int frequency; //Stores the frequency of the service
    double priority;
    ...
    public Image(String job_id, int frequency, double priority, long task_index, double cpu, double ram, double runtime_disk, double storage)
...
class Cache
{
    ...
    String min_priority(HashMap<String,Image> hm)
    { // Function to find service id that has minimum timestamp from the set of images
        ...
        {
            if(entry.getValue().priority < priority_min)
            {
                priority_min = entry.getValue().priority;
                min_pr = entry.getKey();
            }
        }
        return min_pr;
    ...
    double priority_calc(double clock, int frequency, double size)
    {
        if(size == 0)
        {
...
class LRU
{
    void LRU_cache(Cache cache, String drivers, String requests) throws IOException
    {
        double clock = 0;
        ...
        while(sc.hasNextLine() && (count < cache.no_services_max))
        {
            ...
            if(cache.Running.containsKey(service_id)) //if service is cached
            {
                ...
                to_update.priority = cache.priority_calc(clock, to_update.frequency, to_update.Runtime_disk);
            ...
            else if(cache.Free.containsKey(service_id))
            {
                ...
                {
                    replace.frequency = 1;
                    replace.priority = cache.priority_calc(clock, replace.frequency, replace.Runtime_disk);
            ...
            else
            {
                ...
                while(!((CPU_req + CPU_copy <= cache.CPU_max) && (RAM_req + RAM_copy <= cache.RAM_max) && (Disk_storage_req + Disk_storage_curr <= cache.Disk_storage_max)))
                {
                    ...
                    {
                        String to_remove = cache.min_priority(Running_clone); //finding service id with least time stamp
                    ...
                    if(Disk_storage_req + Disk_storage_curr > cache.Disk_storage_max)
                    {
                        ...
                        {
                            String to_remove = cache.min_priority(Running_clone); //finding service id with minimum timestamp
                            Image remove = Running_clone.get(to_remove);
                        ...
                        {
                            String to_remove = cache.min_priority(Free_clone); //finding service with minimum timestamp
                            Image remove = Running_clone.get(to_remove);
                            Runtime_copy -= remove.Runtime_disk;
                            Storage_copy -= remove.Storage;
                            clock_copy += remove.priority;
                ...
                double Pr = cache.priority_calc(clock, 1, Runtime_disk_req);
                cache.Running.put(service_id, new Image(tokens[0], 1, Pr,Long.parseLong(tokens[2]), Double.parseDouble(tokens[3]), Double.parseDouble(tokens[4]), Runtime_disk_req, Storage_req));
        ...
        FileWriter writer = new FileWriter("/home/btech/nityanand.mathur/LRU/Results.csv",true);
...
public class GDSF
{
    public static void main(String[] args) throws IOException 
    {
        ...
        LRU e = new LRU();
        e.LRU_cache(c, drivers, requests);
